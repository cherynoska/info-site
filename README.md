...
1. Canlı kullanım için Decap CMS `git-gateway` backend ile Netlify Identity + Git Gateway akışına göre hazırlanmıştır.
2. Netlify üzerinde ilgili repo bağlanmalı, `Identity` ve `Git Gateway` etkinleştirilmeli, gerekirse GitHub external provider açılmalıdır.
3. Yalnızca belirli kullanıcıların girebilmesi için Netlify Identity kaydı `Invite only` yapılmalı ve sadece istenen e-posta adresi davet edilmelidir.
4. Lokalde test etmek için iki terminal kullanın:
   - Terminal 1: `npm run dev`
   - Terminal 2: `npm run cms`
5. İçerik koleksiyonları:
...
- En sağlam yöntem: Netlify Identity + Git Gateway
- Alternatif: kendi OAuth proxy'nizi host etmek
